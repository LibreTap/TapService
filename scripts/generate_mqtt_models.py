...
def get_schema_files_from_github() -> list[str]:
    ...
    try:
        ...
        print(f"⚠️  Could not fetch schema list from GitHub API: {e}")
...
def download_schemas(temp_dir: Path) -> bool:
    ...
    print("📥 Downloading schemas from LibreTap/mqtt-protocol repository...")
    ...
    if not schema_files:
        print("⚠️  No schema files found via API, will rely on glob pattern after generation")
    ...
    for schema_file in schema_files:
        ...
        try:
            ...
            print(f"   ❌ Failed to download {schema_file}: {e}")
            return False
...
def combine_generated_files(generated_dir: Path, output_file: Path):
    ...
    print("📦 Combining generated files...")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        if not download_schemas(temp_dir):
            print("\n❌ Failed to download schemas from GitHub")
            sys.exit(1)
        ...
        try:
            ...
            print(f"\n✨ Generated models saved to: {output_file}")
            print("\n⚠️  DO NOT edit this file manually!")
            print("   To regenerate: uv run python scripts/generate_mqtt_models.py")
